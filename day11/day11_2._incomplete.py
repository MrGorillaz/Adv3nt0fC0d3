...
def expand_universe(universe):
    ...
    print("expand-Rows")
    for expand_row in expand_row_pos:
        #for i in tqdm(enumerate(range(1_000_000))):
            #universe = np.insert(universe,expand_row,".",axis=1)
        universe = [np.insert(universe,expand_row,".",axis=1) for i in tqdm(range(999_999))]
    
    print("expand-Lines")
    for expand_line in expand_line_pos:
        #for i in tqdm(range(1_000_000)):
         #universe = np.insert(universe,expand_line,".",axis=0)
        universe = [np.insert(universe,expand_line,".",axis=0) for i in tqdm(range(999_999))]
